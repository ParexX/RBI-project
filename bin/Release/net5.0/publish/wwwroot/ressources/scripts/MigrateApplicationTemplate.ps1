...
Write-Output "---Script de migration de la table ApplicationTemplate---"
try {
...
try { 
    try {  
        ...
        Get-SwisData $swis 'SELECT ApplicationTemplateID as ID, Name FROM Orion.APM.ApplicationTemplate' | ForEach-Object {

            $Command.CommandText+= "INSERT INTO ApplicationTemplate VALUES($($_.ID),'$($_.Name)');" 
