...
Write-Output "---script de migration de la table category---"
...
try { 
    try {  
        ...
        Get-SwisData $swis 'SELECT ID, Name FROM Orion.HardwareHealth.HardwareCategory' | ForEach-Object {

            $Command.CommandText+= "INSERT INTO Category VALUES($($_.ID),'$($_.Name)');" 
