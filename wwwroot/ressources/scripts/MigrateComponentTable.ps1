...
try { 
    try {  
        ...
        $Command.CommandText ="DELETE FROM Component;"
        Get-SwisData $swis 'SELECT C.ComponentID as ID, C.ApplicationID, C.Name, C.Status FROM Orion.APM.Component C' | ForEach-Object {

            $Command.CommandText+= "INSERT INTO Component VALUES($($_.ID),$($_.ApplicationID),'$($_.Name)','$($_.Status)');" 
