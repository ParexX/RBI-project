...
try { 
    try {  
        ...
        $Command.CommandText ="DELETE FROM Application;"
        Get-SwisData $swis 'SELECT ApplicationID, ApplicationTemplateID,NodeID, Status FROM Orion.APM.Application' | ForEach-Object {

            $Command.CommandText+= "INSERT INTO Application VALUES($($_.ApplicationID),$($_.ApplicationTemplateID),$($_.NodeID),$($_.Status));" 
