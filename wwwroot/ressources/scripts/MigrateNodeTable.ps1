...
try { 
    try {  
        ...
        $Command.CommandText = "DELETE FROM Node;"
        Get-SwisData $swis 'SELECT N.NodeID ,N.NodeName ,NCP.CDIVALDO,NCP.CodeCS,N.Status FROM Orion.Nodes N LEFT JOIN Orion.NodesCustomProperties NCP ON N.NodeID = NCP.NodeID;' | ForEach-Object {
        $Command.CommandText += "INSERT INTO Node VALUES($($_.NodeID),'$($_.NodeName)','$($_.CDIVALDO)','$($_.CodeCS)',$($_.Status));" 
