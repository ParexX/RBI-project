...
try { 
    try {  
        ...
        $Command.CommandText ="DELETE FROM HardwareInfo;"
        Get-SwisData $swis 'SELECT ID, NodeID, Status, DetailsUrl FROM Orion.HardwareHealth.HardwareInfo' | ForEach-Object {

            $Command.CommandText+= "INSERT INTO HardwareInfo VALUES($($_.ID),$($_.NodeID),$($_.Status),'$($_.DetailsUrl)');" 
